...
if with_config('cublas')
  ...
  $objs = %w[ggml-cuda.o ggml.o ggml-alloc.o llama.o llama_cpp.o]
  $objs << 'k_quants.o' unless with_config('no_k_quants')
end
...
elsif UNAME_M.match?(/armv6/) && try_compile('#include <stdio.h>',
                                             '-mfpu=neon-fp-armv8 -mfp16-format=ieee -mno-unaligned-access')
  $CFLAGS << ' -mfpu=neon-fp-armv8 -mfp16-format=ieee -mno-unaligned-access'
  $CXXFLAGS << ' -mfpu=neon-fp-armv8 -mfp16-format=ieee -mno-unaligned-access'
elsif UNAME_M.match?(/armv7/) && try_compile('#include <stdio.h>',
                                             '-mfpu=neon-fp-armv8 -mfp16-format=ieee -mno-unaligned-access ' \
...
if with_config('cublas')
  File.open('Makefile', 'a') do |f|
    ...
    f.puts "\tnvcc -arch=native --forward-unknown-to-host-compiler -use_fast_math -Wno-pedantic " \
           '-Xcompiler "-fPIC" -c -o $@ $<'
  end
end
